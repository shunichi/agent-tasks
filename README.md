...
このツールは [herdr](https://github.com/shunichi/herdr) (エージェント対応マルチプレクサ) 前提で
作られている (移行の経緯は `docs/herdr-migration.md`)。spawn (別 pane 起動)・セッション状態検出・
...
コンフリクトチェックの対象外になる (コード領域を持たないため)。一覧では `[human]` プレフィックスで
...
agent-tasks serve --addr :8080   # host を省くと 0.0.0.0 = LAN 公開 (認証なし・LAN 内前提)。起動時に
                                 ...
                                 #   /worktime = 稼働時間の時間配分ビュー、/worktime?view=parallel =
                                 #   時間帯別の並列稼働ビュー (PC 限定。ヒートマップ→日別レーン→タスク別詳細)

...
一致しない。そこで突合は 2 経路を併用する:

- **worktree 経路 (spawn)**: hook の cwd の git root basename (`<project>--<NNNN>`) でマーカーを書く。
  spawn 子セッションは worktree 内で動くので、これで自動的に対応づく。
- **session 経路 (同一セッション start)**: メインリポで start してそのまま作業するセッションは cwd が
  worktree の外 (メインリポ) なので worktree 経路では当たらない。そこで hook が `session_id` キーの
  マーカー (cwd 付き) も書き、start 時に `agent-tasks session-link <id>` が「現在 cwd に対応する
  セッション」を逆引きして `<project>--<NNNN>.link.json` に記録する。エージェントは自分の `session_id`
  を直接知る術がない (専用の環境変数が無い) ため、この cwd 逆引きで特定する。

`list` は worktree マーカーと link 経由のマーカーの新しい方を採用する。マーカー・link はいずれも
マシンローカルな揮発情報なので、git 同期されるストアには置かない (state dir に置く)。

> 制限: 複数のセッションが**同じ cwd** (同じメインリポ) で同時に走っていると、session-link の cwd
> 逆引きが最後に活動したセッションを拾うため取り違える可能性がある。並行開発は通常 spawn (worktree
> ごとに別 cwd) で行うため実用上は問題になりにくい。

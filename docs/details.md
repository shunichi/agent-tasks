...
- **クイックトンネル (`trycloudflare.com`) には Access を貼れない**。認証をかけるには
  **named tunnel + Cloudflare 管理下の独自ゾーン**が必須。

具体的な手順 (cloudflared のログイン・named tunnel 作成・DNS 割当・専用 config・Access アプリ作成・
日々の起動・迂回不可の検証・トラブルシュート) は
**[docs/serve-cloudflare-tunnel.md](serve-cloudflare-tunnel.md)** にまとめている。

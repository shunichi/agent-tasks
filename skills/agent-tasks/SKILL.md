...
ユーザーの発言から操作を判定する。引数 (`create`/`list`/`start`/`done`/`block`/`archive`/`unarchive`/`issue`/`sync`/`scaffold`/`recommend`/`batch`) があればそれに従う。
...
1. 候補と現況を集める (既定は現在 project。横断したいときは `--all-projects`):
   ...
   agent-tasks --status in-progress          # 衝突回避の基準 (今動いているもの)
   agent-tasks --status blocked              # 別枠 (下記)
   ```
2. 各 todo 候補を次の観点で評価する (start のコンフリクトチェックと同じ目線):
   - **in-progress との衝突回避 (最優先)**: in-progress タスクの要件・進捗ログを読み、触っている
     ファイル/領域を把握する。候補が**それらと重ならない**ほど高評価 (並行で安全)。重なる候補は
     「in-progress 完了後が安全」として見送り側に回す。
...
6. **セッション状態を紐づける** (任意・冪等。session 状態 hook を導入している場合のみ意味がある)。
   ...
   **取得方法は agent 固有**なので下記から選ぶ:
   ...
   - **セッション名を現在タスクに合わせる (Claude Code, 任意)**: web / スマホアプリのセッション一覧で
     「今どのタスクか」を分かるようにするため、着手時にセッション名を `タスク <NNNN>: <title>` にする。
     Claude Code では **`/rename タスク <NNNN>: <title>`** で変更でき、Claude Desktop / claude.ai/code
     利用時はクラウド側のセッション名にも同期される。ただし `/rename` は**スラッシュコマンドなので
     Claude 自身は自動実行できない** (ツールから呼べない)。そこで着手報告に併せて
     `/rename タスク <NNNN>: <title>` の 1 行を提示し、ユーザーに実行してもらう。
     (spawn で新規に開く子セッションは起動時に `claude -n` で自動命名される。spawn 参照。
     `rename_session` 相当のツール API は現状 Anthropic 側で「実装予定なし」なので自動化はここまで。)
...
4. **メインリポ root で** pane を開き、子セッションを起動して start を指示する。開いた pane の id を
   ...
   tmux send-keys -t "$PANE" "$AGENT -n 'タスク <NNNN>: <title>' 'タスク <NNNN> に着手して'" Enter
...
claude -n 'タスク <NNNN>: <title>' 'タスク <NNNN> に着手して'
```

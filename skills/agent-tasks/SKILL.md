...
0. **まず `agent-tasks session-rename <NNNN>` を叩く (最優先・タスク特定より前)。**
   ...
   まずこれを 1 回実行する。並行セッションを一覧で「今どのタスクか」識別できるよう、表示名を
   `task <NNNN>: <title>` にする。
   ```sh
   agent-tasks session-rename <NNNN>
   ```
   - **herdr 内 (主経路)**: id から title を解決し、`herdr agent rename <自 pane> "task <NNNN>: <title>"`
     で自 pane の herdr ラベルを設定する (send-keys 不要・tmux 非依存)。herdr のサイドバー / モバイル
     attach に反映される。⚠️ これは **herdr 内ラベル**で、claude.ai web/アプリのセッション名は変わらない
     (別 surface)。web/アプリ側命名は spawn 子の `claude -n` (起動時命名) で付く。
   - **herdr 外 (フォールバック)**: tmux 内なら自分の pane へ `/rename …` を `send-keys` で打つ (本物の
     /rename 経路 = claude.ai 名も更新)。tmux 外なら `/rename …` 行を出力するのでユーザーが実行する。
   - **なぜ最初か**: フォールバックの send-keys はこのセッションの入力欄に打つので、**入力欄が空なうち
     (指示直後) が最も競合しにくい**。`session-rename` は id を渡すだけで CLI 内で完結するので内容取得より
     前に呼んでよい (herdr 経路は send-keys を使わないので競合しないが、経路が env で切り替わるため手順は同じ)。
   - 存在しない id ならエラーで何もしない (無害)。human タスクでも任意で叩いてよい。spawn で開く子は
     起動時に `claude -n` で自動命名されるので本手順は不要。

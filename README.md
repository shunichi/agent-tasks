...
- **session 経路 (同一セッション start)**: メインリポで start してそのまま作業するセッションは cwd が
  ...
  マーカー (cwd 付き) も書き、start 時に `agent-tasks session-link <id>` がセッションを
  `<project>--<NNNN>.link.json` に記録する。session_id の特定方法は 2 通り:
...
> 同時に走っていると最後に活動したセッションを拾うため取り違える可能性がある。`--session` で明示すれば
> 回避できる。並行開発は通常 spawn (worktree ごとに別 cwd) で行うため実用上も問題になりにくい。

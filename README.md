...
`doctor` はストアを点検し、同一 project 内で同じ id を持つファイルが複数ある状態 (並行 create の
採番競合で起きる) と、frontmatter の `id:` がファイル名先頭の連番とずれている状態を報告する。
問題があれば終了コード 1 を返すので、CI や着手前チェックに使える。既定は全 project を横断し、
`--project <name>` で 1 project に絞れる。
...
**worktree ごとのマーカー** (`~/.local/state/agent-tasks/sessions/<project>--<NNNN>.json`、
`AGENT_TASKS_STATE_DIR` で変更可) を更新する。突合キーに worktree (hook の cwd の git root basename)
を使うのは、hook が受け取る `session_id` がローカル CLI の UUID で、frontmatter の `session:`
(claude.ai の URL) と一致しないため。start/spawn が作る worktree は 1 タスク = 1 worktree = 1 セッション
なので確実に対応づく。マーカーはマシンローカルな揮発情報なので git 同期されるストアには置かない。

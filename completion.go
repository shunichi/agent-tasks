...
_agent_tasks() {
    ...
    case "$sub" in
        show|edit|open|session-link|session-rename|archive|unarchive|issue|claim|resume|done|block|worktime)
...
%[1]s    )
    ...
    case $sub in
        ...
        claim)
            ...
            ;;
        resume)
            # [<project>] <id> の位置引数 + フラグ (--agent / --session)。
            if [[ ${words[CURRENT]} == -* ]]; then
                _values 'option' \
                    '--agent[記録する agent 名]' \
                    '--session[session URL を記録]' \

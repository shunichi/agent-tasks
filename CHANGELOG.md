...
### Changed
- **`serve` の「時間帯・並列」画面で、詳細ビュー (1日の詳細 — タスク別レーン) の既定日を最新日にした**
  (#0161)。従来は「稼働タスクの種類が最も多い日」を開いていたが、詳細で見たいのは普通「今日/直近どう
  動いたか」で、過去のピーク日が開いていると毎回クリックし直しになっていた。日をクリックして選んだ後に
  その選択が維持される挙動は従来どおり。

### Fixed
- **閉じられなかった working 区間で実稼働時間が暴走する問題を修正した** (#0160)。worktime ログは
  状態遷移の点列でしかなく、終端イベント (idle/done) を取りこぼすと区間が開いたままになる。従来は
  それを窓の終端 (未完了タスクでは現在時刻) まで伸ばしていたため、実測で「9 日間 working」といった
  値になり、横断集計の総計 `12d17h` のうち 1 タスクが `9d13h` を占めていた。取りこぼしは pane 消滅と
  同時の終端イベント (worktime-record が pane から session_id を解決できない) や herdr 自体の終了で
  起き、200 ログ中 24 本で発生していた。
  開いた区間の閉じ位置を **証拠 → 観測 → 推測** の順で決めるようにした:
  1. herdr が「今も working」と報告するセッションは従来どおり伸ばす (観測なので上限を掛けず、
     長時間の自動実行を過小計上しない)。
  2. そうでなければ **agent の transcript の最終活動時刻**で閉じる。transcript は作業中ずっと
     書かれるので (実測: 53 分の working 区間に 598 エントリ)、最終エントリが作業の終了時刻の
     良い近似になる。最終活動が区間開始より前なら 0 長に丸める (= その `working` は実作業を
     伴わない空振りだった)。
  3. transcript も無ければ 30 分で打ち切る (最後の保険)。
  実データでは 151 タスク中 **148 件が不変**で、暴走していた 3 件だけが是正された
  (総計 `12d17h` → `2d15h`)。既存ログの修復は不要 (読み出し時に計算するため)。
  影響: `worktime` / `worktime --all` / `serve` のタイムライン / `tui` の稼働時間表示。
...
- `tui` の詳細ペインでタスク本文を **Markdown として整形表示**するようにした (従来は素のテキスト)。
  見出し・箇条書き (ぶら下げインデント / ネスト)・コードブロック・引用・強調・インラインコード・
  リンクを色と字下げで描き分ける。ソース側で手折返しされた段落はいったん連結してからペイン幅で
  折り直すので、リサイズしても行の長短が揃う。frontmatter は Markdown に通さず `key: value` の
  まま淡色で出す (`---` が水平線に化けるのを避けるため)。日本語の折返しは行頭禁則 (`、` `。`) を
  避け、幅計算は一覧テーブルと同じ go-runewidth に揃えた。

...
0. **まず `agent-tasks session-rename <NNNN>` を叩く (Claude Code。最優先・タスク特定より前)。**
   「start <NNNN>」と指示されたら、**対象タスクの特定・二重着手/コンフリクトチェックより前に**、
   まずこれを 1 回実行する。web / スマホアプリのセッション一覧 (Bridge 同期先) で「今どのタスクか」が
   分かるよう、セッション名を `task <NNNN>: <title>` にする。
   ```sh
   ...
   - これが id から title を解決し、**tmux 内なら自分の pane (`$TMUX_PANE`) へ `/rename task <NNNN>: <title>`
     ...
     それをユーザーに提示して実行してもらう。
   - **なぜ最初か (競合回避)**: send-keys はこのセッションの入力欄に打ち込むので、**入力欄が空なうち
     (指示直後、ユーザーがまだ何も打っていない) が最も競合しにくい**。タスク内容取得やチェックを挟むと、
     その間にユーザー入力の窓が開く。`session-rename` は id を渡すだけで CLI 内で title 解決〜送出まで
     完結する (Claude 側は内容を読まなくてよい) ので、内容取得より前に呼んで問題ない。
   - 存在しない id ならエラーで何もしない (無害)。二重着手の可能性があるタスクに先に rename する懸念は
     あるが、rename は cosmetic で巻き戻せるため、**競合回避 (入力欄が空なうちに送る) を優先する**。
   - (送られた `/rename` はこのターン終了後に発火する。作業は壊さない。spawn で開く子セッションは
     起動時に `claude -n` で自動命名されるので本手順は不要。`/rename` を起こすツール/フックは現状
     send-keys がほぼ唯一の自動手段。将来 `SessionRename` 相当が入れば不要: Issue anthropics/claude-code
     #54325 / #29355。human タスクでも任意で叩いてよい。)
...
5. **残りの frontmatter を更新する** (status/agent/started_at は手順2の claim が済ませている):
   ...
   - `session:` 自分のセッション URL が分かれば記録 (Claude Code なら会話フッタの `Claude-Session` URL。
     claim に `--session` で session_id を渡した場合と別物: こちらは人が開く URL)
...
## spawn — 別 pane で新セッションを開いて start させる (herdr, fire-and-forget)
...
**herdr 前提 (全面移行)**。旧来の tmux `split-window` + `send-keys` ハックは `agent-tasks spawn` CLI に
集約された (herdr `agent start` を使う)。agent はこのコマンドを 1 回叩くだけでよい。
...
3. **親はここで完了**。worktree 作成も worktree-init もポーリングもしない。子の `start` が
...
> herdr 外で実行するとエラーになる (全面移行版は herdr 必須)。herdr 内で起動していることが前提。
